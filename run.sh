...
PIPE_HOST=${PIPE_HOST:-0.0.0.0}
PIPE_PORT=${PIPE_PORT:-8002}
BACK_HOST=${BACK_HOST:-0.0.0.0}
BACK_PORT=${BACK_PORT:-8001}
FRONT_HOST=${FRONT_HOST:-0.0.0.0}
FRONT_PORT=${FRONT_PORT:-8000}
PIPE_URL=${PIPE_URL:-http://localhost:$PIPE_PORT}
BACK_URL=${BACK_URL:-http://localhost:$BACK_PORT}

export PIPE_HOST PIPE_PORT PIPE_URL
export BACK_HOST BACK_PORT
export FRONT_HOST FRONT_PORT BACK_URL

trap 'kill $(jobs -p) 2>/dev/null' EXIT INT TERM

echo "pipeline → http://$PIPE_HOST:$PIPE_PORT"
echo "backend  → http://$BACK_HOST:$BACK_PORT"
echo "frontend → http://$FRONT_HOST:$FRONT_PORT"

.env-back/bin/uvicorn  api:app  --app-dir src/pipeline --host "$PIPE_HOST" --port "$PIPE_PORT" &
.env-back/bin/uvicorn  app:app  --app-dir src/back     --host "$BACK_HOST" --port "$BACK_PORT" &
